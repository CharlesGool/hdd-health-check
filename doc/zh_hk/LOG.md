...
本文記錄已採納的歷史決策、已知限制及發佈歷史；v2.2.0 已在本地整合，但尚未發佈。
...
- [ ] 此整合版本尚未驗證實際 HDD 整合、root／套件安裝及 systemd 行為；先前 v1.0.0 發佈檢查亦沒有實體 HDD 和 SMART 數據（只檢查了語法及說明）。據報原始版本在標準化前曾被使用，但不能代替受控的硬件測試。
- [ ] 啟發式健康權重並非經校準的故障概率；SAS/SCSI 評分的測試程度低於 ATA，USB/RAID SMART 透傳亦可能失敗。依賴廠商資料的溫度報告尚不完整。
- [ ] 沒有結構化 JSON/CSV 輸出。既有但不符合規範的 v2.2 狀態會被拒絕；自訂 `TMPDIR` 的背景狀態偵測及安全停止、異常表面掃描進度現已通過隔離模擬測試；實際 systemd 及 HDD 行為仍未驗證。
...
| 目前整合：採用所提供的 v2.2.0 行為，不保證兼容 v1，並保留現有 MIT 授權。 | 新腳本加入持久狀態及可選的背景工作；`-w` 會被忽略。此次整合並非發佈、建立 tag 或硬件驗證聲明。 |
...
- 主機狀態及日誌分開寫入；提供的 v2.2.0 整合版本限制了狀態／計劃解析以及路徑／裝置處理。這次並未執行實際 HDD、root 或 systemd 整合測試。
