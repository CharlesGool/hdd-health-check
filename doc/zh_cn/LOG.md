...
本文档记录已采纳的历史决策、已知局限及发布历史；源码已在 GitHub 公开提供，但 v2.2.0 尚无标签或 GitHub Release。
...
- [ ] 没有结构化 JSON/CSV 输出。预先存在但不符合格式的 v2.2 状态会被拒绝；自定义 `TMPDIR` 下的后台状态检测与安全停止、异常盘面进度现已通过隔离模拟测试。
...
- 主机状态和日志分别写入；提供的 v2.2.0 整合版对状态／计划解析以及路径／设备处理施加约束。用户报告已在实机测试 v2.2.0，但未提供设备、环境或测试范围；不能确认 root／软件包安装或 systemd 行为已测。
