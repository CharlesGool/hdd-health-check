...
本文件保存已採納的歷史決策、已知限制與版本歷程；原始碼已在 GitHub 公開提供，但 v2.2.0 尚無標籤或 GitHub Release。
...
- [ ] 沒有結構化 JSON/CSV 輸出。不符合格式的既有 v2.2 狀態會遭拒；自訂 `TMPDIR` 時的背景狀態偵測與安全停止、異常磁碟表面進度現已通過隔離模擬測試。
...
- 主機狀態與日誌分別寫入；所提供的 v2.2.0 整合限制了狀態／計畫資料的解析，以及路徑／裝置處理。使用者表示已在實機測試 v2.2.0，但未提供裝置、環境或測試範圍；無法確認 root／套件安裝或 systemd 行為已測。
